...
- 读取专项审查 skill 时，优先参考其 `严重级别参考`
- 合并发现时，可借用各 skill 的 `示例发现` 作为“这类问题通常为何严重”的快速解释模板
- 若某条发现触犯了专项 skill 的 `不要这样报` 规则，应先降级、归并或退回补证据，而不是直接上升为阻塞结论
- 跨司问题若根因相同，优先汇总成一个主问题，再在 `evidence` 中引用多个发现编号
